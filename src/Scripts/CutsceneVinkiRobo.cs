...
namespace Vinki
{
    public class CutsceneVinkiRobo : UpdatableAndDeletable
    {
        public CutsceneVinkiRobo(Room room)
        {
            ...
            // Dead scav (lol)
            AbstractCreature abstractCreature = new(room.world, StaticWorld.GetCreatureTemplate(DLCSharedEnums.CreatureTemplateType.ScavengerElite), null, new WorldCoordinate(room.abstractRoom.index, scavX, scavY, -1), room.game.GetNewID());
